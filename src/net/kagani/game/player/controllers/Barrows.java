...
import net.kagani.game.npc.others.BarrowsBrother;
...
public final class Barrows extends Controller {
	...
	private static enum Hills {
		...
		AHRIM_HILL(new WorldTile(3564, 3287, 0), new WorldTile(3557, 9703, 3)), DHAROK_HILL(
				...
				new WorldTile(3556, 3296, 0), ENABLE_AKRISAE ? new WorldTile(
						4077, 5710, 0) : new WorldTile(3578, 9706, 3));
	...
	private static final Item[] COMMUM_REWARDS = { new Item(558, 1795),
			new Item(562, 473), new Item(560, 491), new Item(565, 464),
			new Item(4740, 588), new Item(30004, 1) };
	...
	private static final Item[] BARROW_REWARDS = { new Item(4708, 1),
			...
			new Item(4757, 1), new Item(4759, 1), new Item(21736, 1),
			new Item(21744, 1), new Item(21752, 1), new Item(21760, 1) };
	...
	public void drop(Item item) {
		...
		// Settings.getDropQuantityRate(player) : item.getAmount()) + 1);
	...
	public void sendReward() {
		...
		drop(new Item(995, Utils.random(50307))); // money reward at least
		// always
		player.getTimersManager().increaseKills(RecordKey.THE_BARROWS_BROTHERS,
				false);
	}
	...
	public void start() {
		if (player.getHiddenBrother() == -1 || player.getHiddenBrother() == 6)
			player.setHiddenBrother(Utils.random(ENABLE_AKRISAE ? player
					.getKilledBarrowBrothers().length : 6));
