...
import net.kagani.game.player.actions.mining.DungeoneeringMining.DungeoneeringRocks;
import net.kagani.game.player.actions.mining.MiningBase.PickAxeDefinitions;
...
import net.kagani.game.player.content.dungeoneering.Door;
...
import net.kagani.game.player.content.dungeoneering.DungeonResourceShop;
...
import net.kagani.game.player.content.dungeoneering.VisibleRoom;
...
import net.kagani.game.player.content.dungeoneering.skills.DungeoneeringFarming;
...
import net.kagani.game.player.content.dungeoneering.skills.DungeoneeringFarming.Harvest;
...
public class DungeonController extends Controller {
	...
	public boolean processNPCClick2(final NPC npc) {
		...
		if (npc instanceof Familiar) {
			...
			// DungeonResourceShop.openResourceShop(player,
			// dungeon.getParty().getComplexity());
			return false;
		}
		return true;
	...
	public boolean handleItemOnObject(WorldObject object, Item item) {
		...
		case "anvil":
			 ...
			 * Smithing.sendForgingInterface(player, index, true); break; } }
			 */
			return true;
		}
		return true;
	...
	public boolean canUseItemOnItem(Item itemUsed, Item usedWith) {
		if (dungeon == null || !dungeon.hasStarted()
				|| dungeon.isAtRewardsScreen())
			return false;
		if (itemUsed.getId() == LeatherCraftingD.DUNG_NEEDLE
				|| usedWith.getId() == LeatherCraftingD.DUNG_NEEDLE) {
	...
	public boolean processButtonClick(int interfaceId, int componentId,
		...
		if (dungeon.isAtRewardsScreen()) {
			...
			if (interfaceId == 1465) {
				if (componentId == 24 || componentId == 38) {
				...
				if (packetId == WorldPacketsDecoder.ACTION_BUTTON1_PACKET) {
					...
					if (leatherIndex != -1) {
						player.getDialogueManager().startDialogue(
								"LeatherCraftingD", leatherIndex, true);
						return false;
					}
				 ...
				 * 15); else if (componentId == 39 || componentId == 40)
				 * stoneTeleport(componentId == 40); else if (componentId == 38)
				 * { if (packetId == WorldPacketsDecoder.ACTION_BUTTON2_PACKET)
				 * { //Quick cast: delete current gatestone without asking
				 * player.getInventory().deleteItem(DungeonConstants.GATESTONE,
				 * 1); if (gatestone != null) { FloorItem item =
				 * World.getRegion(
				 * gatestone.getRegionId()).getGroundItem(DungeonConstants
				 * .GATESTONE, gatestone, player); if (item == null) return
				 * false; World.removeGroundItem(player, item, false);
				 * setGatestone(null); } } if (!canCreateGatestone()) { //TODO:
				 * Dialogue to destroy current gatestone return false; }
				 * player.getInventory().addItem(DungeonConstants.GATESTONE, 1);
				 * player.getSkills().addXp(Skills.MAGIC, 43.5);
				 * player.setNextAnimation(new Animation(713));
				 * player.setNextGraphics(new Graphics(113)); } else
