...
public class Dicing {
	...
	public static void friendsRoll(final Player player, final int itemId,
		...
		if (chat == null) {
			player.getPackets().sendGameMessage(
					"You need to be in a friends chat to use this option.");
			return;
		}
		if (chat.equals(Settings.HELP_ACCOUNT)) {
			player.getPackets().sendGameMessage(
					"You cannot dice in this friends chat.");
			return;
		}
		player.lock(2);
		...
		WorldTasksManager.schedule(new WorldTask() {
			...
			public void run() {
				player.getInventory().addItem(itemId, 1);
				chat.sendLocalMessage("Friends Chat channel-mate <col=db3535>"
