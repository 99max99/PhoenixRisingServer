...
public class AuraManager implements Serializable {
	...
	public void process() {
		...
		if (activation - Utils.currentTimeMillis() <= 60000 && !warned) {
			player.getPackets()
					.sendGameMessage(
							player.isPlatinumMember()
									|| player.isDiamondMember() ? "Your aura will deplete in 1 minute. You will need to reactivate it."
	...
	public long getCoolDown(int aura) {
		...
		if (coolDown == null || player.isPlatinumMember()
	...
	public void sendAuraRemainingTime() {
		if (!isActivated()) {
			...
			if (!player.isPlatinumMember() || !player.isGoldMember()) {
	...
	/*
	 * 16449 - Corruption 16464 - Greater corruption 16429 - SlayerMasterD
	 * corruption 68615 - Supreme corruption
	 * 
	 * 16465 - Salvation 16524 - Greater salvation 16450 - SlayerMasterD
	 * salvation 68611 - supreme salvation.
	 * 
	 * 68605 - Harmony. 68610 - Greater harmony. 68607 - SlayerMasterD harmony.
	 ...
	 * return seconds
	 */
	public static int getActivationTime(Player player, int aura) {
		switch (aura) {
		case 20958:
	...
	public static int getCooldown(Player player, int aura) {
		switch (aura) {
		case 20962:
		case 22270:
		case 20967:
