...
public class StealingCreationController extends Controller {
	...
	public void sendInterfaces() {
		...
		player.getPackets().sendCSVarInteger(558, (int) ((game.getEndTime() - Utils.currentTimeMillis()) / 600)); // sync
		// time
	...
	public void process() {
		if (game == null
				|| (getPlayer().getX() < game.getArea().getMinX() || getPlayer().getX() > game.getArea().getMaxX()
						|| player.getY() < game.getArea().getMinY() || player.getY() > game.getArea().getMaxY())) {
	...
	public boolean canHit(Entity target) {
		if (target instanceof Player) {
			...
			if (playerTarget.getEquipment().getCapeId() == player.getEquipment().getCapeId()
					|| Helper.withinSafeArea(playerTarget, game.getArea(), !getTeam())) {
				return false;
			}
		...
		if (target instanceof Player) {
			...
			if (playerTarget.getEquipment().getCapeId() == player.getEquipment().getCapeId()) {
				...
				return false;
					...
					&& (Long) target.getTemporaryAttributtes().get("in_kiln") >= Utils.currentTimeMillis()) {
			...
			if (target.isDead() || player.isDead()) {
				...
				if (target.getInventory().addItem(item)) {
					...
					target.getPackets().sendGameMessage(
							Utils.formatPlayerNameForDisplay(player.getDisplayName()) + " has given you an item.");
					return false;
				} else {
					player.getPackets().sendGameMessage(Utils.formatPlayerNameForDisplay(player.getDisplayName())
	...
	public boolean canPlayerOption3(final Player target) {
		...
		if (PICKPOCK_DELAY != null && PICKPOCK_DELAY > Utils.currentTimeMillis()) {
			return false;
			...
			return false;
				...
				&& (Long) target.getTemporaryAttributtes().get("in_kiln") >= Utils.currentTimeMillis()) {
			player.getPackets().sendGameMessage("You cannot seem to access " + target.getDisplayName()
			...
			player.getPackets()
					.sendGameMessage("You need a theiving level of at least "
							+ (target.getSkills().getLevel(Skills.THIEVING) - 20) + " to pickpocket "
							+ Utils.formatPlayerNameForDisplay(target.getDisplayName() + "."));
			return false;
		} else if (target.getInventory().getFreeSlots() == 28) {
			player.getPackets().sendGameMessage(Utils
					.formatPlayerNameForDisplay(target.getDisplayName() + " appears to have nothing in his pockets."));
			return false;
		} else if (player.getInventory().getFreeSlots() == 0) {
			player.getPackets()
					.sendGameMessage("You don't have enough space in your inventory to steal from your target.");
			return false;
		...
		player.getTemporaryAttributtes().put("PICKPOCK_DELAY", Utils.currentTimeMillis() + 1000);
		int level = Utils.random(thievingLevel);
		double ratio = level / (Utils.random(target.getSkills().getLevel(Skills.THIEVING)) + 3);
		if (Math.round(ratio * thievingLevel) > target.getSkills().getLevel(Skills.THIEVING)) {
			...
			target.getPackets().sendGameMessage(player.getDisplayName() + " has stolen a item from you.");
			player.setNextAnimation(new Animation(881));
			player.getPackets().sendGameMessage("You have successfully pickpocketed an item from "
					+ Utils.formatPlayerNameForDisplay(target.getDisplayName()) + "'s pockets!");
		}
		return false;
	}
	...
	public boolean keepCombating(Entity target) {
		if (target instanceof Player) {
			...
			if (playerTarget.getAppearence().isNPC()) {
				...
				return false;
			} else if (Helper.withinSafeArea(playerTarget, game.getArea(), !getTeam())
					|| Helper.withinSafeArea(player, game.getArea(), getTeam())) {
				return false;
			}
		}
	...
	public boolean canTakeItem(FloorItem item) {
		...
		int clayQuality = 1;
		if (nameIndex != -1)
			clayQuality = name.contains("potion") ? 1
					: Integer.parseInt(name.substring(nameIndex).replace("(class ", "").replace(")", ""));
		...
		if (interfaceId == 813) {
			...
			if (item != null) {
				...
				if (itemName.contains("food (class")) {
					doFoodEffect(item, Integer.parseInt(item.getName().substring(item.getName().indexOf("(class"))
					...
					for (String name : Skills.SKILL_NAME) {
						int doses = Integer
								.parseInt(itemName.substring(itemName.indexOf("(")).replace("(", "").replace(")", ""));
						String skill = superPotion
								? item.getName().toLowerCase().replace("super ", "").replace(" (" + doses + ")", "")
								: item.getName().toLowerCase().replace(" potion (" + doses + ")", "");
						if (!name.toLowerCase().equals(skill)) {
							index++;
							continue;
						}
						player.getPackets().sendGameMessage("You drink a dose of the "
								+ item.getName().toLowerCase().replace("(" + doses + ")", "") + ".");
						if (doses == 1) {
							player.getInventory().deleteItem(item);
							player.getPackets()
									.sendGameMessage("The glass shatters as you drink the last dose of the potion.");
						} else {
						...
						if (!skill.equals("prayer")) {
							...
							player.getPrayer().restorePrayer((int) (Math.floor(
									player.getSkills().getLevelForXp(Skills.PRAYER) * .5 + (superPotion ? 250 : 200))));
	...
	public boolean processObjectClick1(final WorldObject object) {
		...
		if (object.getId() == Helper.KILN) {
			...
			return false;
		} else if ((!getTeam() && (object.getId() == Helper.BLUE_DOOR_1 || object.getId() == Helper.BLUE_DOOR_2))
				|| (getTeam() && (object.getId() == Helper.RED_DOOR_1 || object.getId() == Helper.RED_DOOR_2))) {
			...
			return false;
		} else if (object.getId() == 39602 || object.getId() == 39613 || object.getId() == 39612
				|| object.getId() == 39611) {
			boolean isWall = object.getId() == 39613 || object.getId() == 39612 || object.getId() == 39611;
			if (isWall) {
				if (player.getSkills().getLevel(Skills.AGILITY) < 60) {
					player.getPackets()
							.sendGameMessage("You need to have an Agility level of 60 to clim over the wall.");
					return false;
				}
			}
			...
			final int clayQuality = Integer
					.parseInt(name.substring(name.indexOf("(class")).replace("(class ", "").replace(")", "")) - 1;
			final CreationChunks skill = CreationChunks
					.valueOf(name.replace(" (class " + (clayQuality + 1) + ")", "").toUpperCase());
			...
			player.getActionManager().setAction(new Action() {
				...
				public boolean start(Player player) {
					if (name.contains(("empty")))
						...
						player.getPackets().sendGameMessage("You need a " + Skills.SKILL_NAME[skill.getRequestedSkill()]
								+ " level of " + clayQuality * 20 + " to collect this level of clay.");
						return false;
					}
					itemLoop: for (int index = 4; index >= 0; index--) {
						...
						if (player.getEquipment().getWeaponId() == bestItem.getId()
								|| player.getInventory().containsItem(bestItem.getId(), bestItem.getAmount())) {
				...
				public boolean process(Player player) {
					if (game.isEmpty(flagX, flagY) || player.getInventory().getFreeSlots() == 0)
						return false;
					player.setNextAnimation(bestItem != null ? new Animation(skill.getBaseAnimation() + itemTier)
							: new Animation(10602));
				...
				private int getActionDelay() {
					...
					int mineTimer = baseTime - player.getSkills().getLevel(skill.getRequestedSkill())
							- (bestItem == null ? 1 : Helper.TOOL_TIERS[itemTier]);
					if (mineTimer < 2)
						mineTimer = 2;
	...
	public boolean processObjectClick2(WorldObject object) {
		...
		if (object.getId() == Helper.EMPTY_BARRIER1 || object.getId() == Helper.EMPTY_BARRIER2
			...
			WorldTasksManager.schedule(new WorldTask() {
				...
				public void run() {
					if (step == 0) {
						...
						if (player.getInventory().removeItems(new Item(Helper.BARRIER_ITEMS[t], 4))
		...
		if (isFriendlySCGate || isFriendlySCWall) {
			...
			synchronized (game.getLock()) {
				...
				if (!player.getInventory().containsItem(Helper.BARRIER_ITEMS[tier - 1], 1)) {
					player.getPackets()
							.sendGameMessage("You don't have enough barriers of required type to repair this barrier.");
					return false;
				}
				player.lock(2);
				WorldTasksManager.schedule(new WorldTask() {
					...
					public void run() {
						if (step == 0) {
							...
							if (player.getInventory().removeItems(new Item(Helper.BARRIER_ITEMS[tier - 1], 1))
			...
			public void run() {
				...
				if (step == 0) {
					...
					p.setNextForceMovement(
							new ForceMovement(fromTile, 0, faceTile, 1, Helper.getFaceDirection(faceTile, p)));
			...
			public void run() {
				if (loop == 0) {
					...
					if (score != null && killer != null) {
						...
						killer.getPackets().sendGameMessage("You have killed " + player.getDisplayName()
								+ ", you now have " + killerScore.getKilled() + " kills.");
	...
	public void processKilnExchange(int componentId, int packetId) {
		int quality = player.getTemporaryAttributtes().get("sc_kiln_quality") != null
				? (int) player.getTemporaryAttributtes().get("sc_kiln_quality") : 0;
